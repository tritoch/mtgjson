...
setsToDo.serialForEach(
	function(arg, subcb) {
		...
		if(targetSet.isMCISet) {
			base.error("Set %s is an MCI set, cannot retrieve other languages.", arg);
			return setImmediate(subcb);
		}
		...
		tiptoe(
			function loadSet() {
				fs.readFile(path.join(__dirname, '..', 'json', targetSet.code.toLowerCase() + '.json'), this);
			},
			function process(setRaw) {
				set = JSON.parse(setRaw);
				rip.ripLang(set, this);
			},
			function save(set) {
